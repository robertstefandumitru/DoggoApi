...
namespace DoggoApi.Services
{
    public class DoggoService : BaseService
    {
        ...
        private const string GET_BREED_IMAGES_ENDPOINT = "breed/{0}/images/random/10";
        private const string GET_SUB_BREED_IMAGES_ENDPOINT = "breed/{0}/{1}/images/random/10";
        ...
        public static async Task<BreedImagesDto> GetBreedImages(string breedName)
        {
            var endpoint = string.Format(GET_BREED_IMAGES_ENDPOINT, breedName);
            var url = Path.Combine(BASE_API_URL, endpoint);
            var result = await RestService.GetAsync<BreedImagesDto>(url);
            return result;
        }

        public static async Task<BreedImagesDto> GetSubBreedImages(string breedName, string subBreedName)
        {
            var endpoint = string.Format(GET_SUB_BREED_IMAGES_ENDPOINT, breedName, subBreedName);
            var url = Path.Combine(BASE_API_URL, endpoint);
            var result = await RestService.GetAsync<BreedImagesDto>(url);
            return result;
        }
